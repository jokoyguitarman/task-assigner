-- Corrected fix for all admin RLS permissions
-- This script handles the correct column names
...
-- Simplified outlet user policy (without outlet_id reference)
CREATE POLICY "Outlet users can view outlets" ON outlets
    FOR SELECT
    TO authenticated
    USING (
        EXISTS (
            SELECT 1 FROM users 
            WHERE users.id = auth.uid() 
            AND users.role IN ('outlet', 'staff')
        )
    );
